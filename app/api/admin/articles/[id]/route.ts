  ...
  try {
    ...
    try {
      ...
      if (syncResult.success) {
        ...
        console.warn(
          "⚠️ Auto-sync failed, falling back to manual update:",
          syncResult.error
        );

        // Fallback: Manual update of optimized fallback
        const { loadOptimizedFallback } = await import(
...
export async function DELETE(
  ...
  try {
    ...
    return NextResponse.json({
      success: true,
      message:
        "Article deleted successfully from both Supabase and local cache!",
    });
  } catch (error) {
