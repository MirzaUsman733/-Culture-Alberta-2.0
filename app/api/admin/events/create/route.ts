  ...
  try {
    ...
    try {
      ...
      const { data: supabaseResult, error: supabaseError } = await supabase
        ...
        .select(
          "id, title, excerpt, description, category, location, event_date, event_end_date, image_url, status, organizer, venue_address, website_url, price, currency, created_at"
        )
        .single();

      if (supabaseError) {
        console.error(
          `❌ Admin API: Supabase creation failed:`,
          JSON.stringify(supabaseError, null, 2)
        );
        console.error(
          `❌ Event data sent:`,
          JSON.stringify(supabaseEventData, null, 2)
        );
        throw new Error(`Supabase creation failed: ${supabaseError.message}`);
      }

      console.log(
        `✅ Admin API: Event successfully created in Supabase with ID: ${supabaseResult.id}`
      );
      ...
      console.warn(
        `⚠️ Admin API: Supabase creation failed, falling back to local creation:`,
        supabaseError
      );
      ...
      console.log(
        `⚠️ Admin API: Event ${eventId} created in fallback only (Supabase unavailable)`
      );
