...
import { getProductionCacheSettings } from "./production-optimizations";
...
import { trackResourceUsage } from "./vercel-optimizations";
...
function ensureImageFields(fields: string): string {
  ...
  if (
    fields.includes(IMAGE_FIELDS.IMAGE_URL) &&
    fields.includes(IMAGE_FIELDS.IMAGE)
  ) {
    return fields;
  }
  ...
  if (process.env.NODE_ENV === "development") {
    console.warn(
      "⚠️ Image fields were missing from query, automatically added them:",
      fields
    );
  }
  return fields;
...
function validateImageUrl(
  ...
  if (!isValidUrl) {
    if (process.env.NODE_ENV === "development") {
      console.warn(
        `⚠️ Invalid image URL for article "${articleTitle}": ${imageUrl}`
      );
    }
    return undefined;
  }
  ...
  if (imageUrl && process.env.NODE_ENV === "development") {
    const truncatedUrl = imageUrl.startsWith("data:")
      ? `${imageUrl.substring(0, 30)}...${imageUrl.substring(
          imageUrl.length - 10
        )}`
  ...
  try {
    ...
    if (error) {
      ...
      if (error.message && error.message.includes("does not exist")) {
        console.error(
          "Articles table does not exist in Supabase. Please run the create-articles-table.sql script."
        );
        return false;
      }
...
export async function getHomepageArticles(): Promise<Article[]> {
  ...
  try {
    ...
    console.log(
      "🔄 UPDATING: Cache is stale, fetching fresh data from Supabase..."
    );

    if (!supabase) {
      console.error("Supabase client is not initialized");
      console.log("Falling back to file system");
      return fileArticlesModule
        ? await fileArticlesModule.getAllArticlesFromFile()
        : [];
    }
    ...
    const fields = ensureImageFields(
      "id, title, excerpt, category, categories, created_at, updated_at, trending_home, trending_edmonton, trending_calgary, featured_home, featured_edmonton, featured_calgary, type, status, author, location, tags"
    );
    ...
    while (attempts < maxAttempts) {
      attempts++;
      console.log(
        `🔄 Attempt ${attempts}/${maxAttempts} to fetch from Supabase...`
      );
      ...
      try {
        ...
        if (!error) {
          ...
          console.warn(
            `❌ Supabase attempt ${attempts} failed:`,
            error.message
          );
        ...
        console.warn(
          `❌ Supabase attempt ${attempts} threw error:`,
          retryError
        );
        if (attempts < maxAttempts) {
          console.log(`⏳ Waiting 2 seconds before retry...`);
    ...
    if (error || !data) {
      console.warn(
        `🚨 All ${maxAttempts} Supabase attempts failed, falling back to file system`
      );
      console.log("Falling back to file system");
      return fileArticlesModule
        ? await fileArticlesModule.getAllArticlesFromFile()
    ...
    console.log(
      "✅ SMART CACHE: Successfully fetched homepage articles from Supabase:",
      data?.length || 0,
      "articles"
    );
...
export async function getAdminArticles(
  ...
  try {
    ...
    const supabasePromise = supabase
      .from("articles")
      .select(
        "id, title, category, location, author, created_at, updated_at, status, type, featured_home, featured_edmonton, featured_calgary, date"
      )
      .order("created_at", { ascending: false })
    ...
    const { data, error } = (await Promise.race([
      ...
      new Promise(
        (_, reject) =>
          setTimeout(
            () => reject(new Error("Supabase timeout")),
            getProductionCacheSettings().timeoutDuration * 1.5
    ...
    console.log(
      "Successfully fetched admin articles from Supabase:",
      data?.length || 0,
      "articles"
    );
...
export async function getEventsArticles(): Promise<Article[]> {
  try {
    ...
    if (
      cityArticlesCache.has("events") &&
      now - eventsCacheTime < getCacheDuration()
    ) {
      console.log(
        "Returning cached events articles:",
        cityArticlesCache.get("events")?.length || 0,
        "articles"
      );
    ...
    if (shouldUseFileSystem()) {
      ...
      const fileArticles = (await fileArticlesModule)
        ? await fileArticlesModule.getAllArticlesFromFile()
        : [];

      // Filter file articles for events only
      const filteredFileArticles = fileArticles.filter(
        (article: any) =>
      ...
      if (filteredFileArticles.length > 0) {
        console.log(
          `Found ${filteredFileArticles.length} events in file system out of ${fileArticles.length} total`
        );
    ...
    if (!supabase) {
      ...
      console.log(
        `Supabase fallback: Found ${filteredFileArticles.length} events out of ${fileArticles.length} total`
      );
      return filteredFileArticles;
    }
    ...
    const fields = ensureImageFields(
      "id, title, excerpt, category, categories, location, created_at, trending_home, trending_edmonton, trending_calgary, featured_home, featured_edmonton, featured_calgary"
    );
    ...
    const { data, error } = (await Promise.race([
      ...
      new Promise(
        (_, reject) =>
          setTimeout(
            () => reject(new Error("Supabase timeout")),
            getProductionCacheSettings().timeoutDuration
          ) // Dynamic timeout based on environment
      ),
    ])) as any;
    ...
    console.log(
      "Successfully fetched events articles from Supabase:",
      data?.length || 0,
      "articles"
    );
    ...
    console.log(
      `File system fallback: Found ${filteredFileArticles.length} events out of ${fileArticles.length} total`
    );
    return filteredFileArticles;
  }
}
...
export async function getCityArticles(
  ...
  try {
    ...
    if (
      cityArticlesCache.has(city) &&
      now - cityCacheTime < getCacheDuration()
    ) {
      console.log(
        `✅ Returning cached ${city} articles:`,
        cityArticlesCache.get(city)?.length || 0,
        "articles"
      );
    ...
    if (shouldUseFileSystemFirst() && fileArticlesModule) {
      console.log(
        `🚀 SPEED: Using file system as primary source for ${city} articles`
      );
      try {
        ...
        if (filteredFileArticles.length > 0) {
          console.log(
            `✅ Found ${filteredFileArticles.length} ${city} articles in file system`
          );
          // Cache the filtered results
        ...
        console.warn(
          `⚠️ File system failed for ${city}, falling back to Supabase:`,
          fileError
        );
      }
    }
    ...
    if (shouldUseFileSystem()) {
      console.log(
        `Build time detected, using file system for ${city} articles`
      );
      const fileArticles = (await fileArticlesModule)
        ? await fileArticlesModule.getAllArticlesFromFile()
        : [];
    ...
    if (articlesCache && now - cacheTimestamp < getCacheDuration()) {
      ...
      if (filteredFromHomepage.length > 0) {
        console.log(
          `Found ${filteredFromHomepage.length} ${city} articles from homepage cache`
        );
        // Cache the filtered results
    ...
    if (!supabase) {
      ...
      console.log(
        `Supabase fallback: Found ${filteredFileArticles.length} ${city} articles out of ${fileArticles.length} total`
      );
      return filteredFileArticles;
    }
    ...
    const fields = ensureImageFields(
      "id, title,  excerpt, category, location, created_at, trending_home, trending_edmonton, trending_calgary, featured_home, featured_edmonton, featured_calgary"
    );
    ...
    const supabasePromise = supabase
      ...
      .select(fields)
      .or(
        `category.ilike.%${city}%,location.ilike.%${city}%,title.ilike.%${city}%`
      )
      .order("created_at", { ascending: false })
      .limit(30); // Reduced limit for faster loading
    ...
    console.log(
      `Successfully fetched ${city} articles from Supabase:`,
      data?.length || 0,
      "articles"
    );
    ...
    console.log(
      `File system fallback: Found ${filteredFileArticles.length} ${city} articles out of ${fileArticles.length} total`
    );
    return filteredFileArticles;
  }
}
...
export async function getAllArticles(): Promise<Article[]> {
  try {
    ...
    console.log(
      "🚀 FORCING FRESH DATA FETCH - Cache disabled for production fix"
    );
    const now = Date.now();
    // if (articlesCache && (now - cacheTimestamp) < getCacheDuration()) {
    ...
    if (shouldUseFileSystemFirst() && fileArticlesModule) {
      ...
      try {
        ...
        console.warn(
          "⚠️ File system failed, falling back to Supabase:",
          fileError
        );
      }
    }
    ...
    console.log(
      "⚡ FAST: Attempting to fetch articles from Supabase (NO CONTENT)..."
    );
    ...
    const fields = ensureImageFields(
      "id, title, excerpt, category, categories, location, author, tags, type, status, created_at, updated_at, trending_home, trending_edmonton, trending_calgary, featured_home, featured_edmonton, featured_calgary"
    );
    ...
    console.log(
      "Successfully fetched articles from Supabase:",
      data?.length || 0,
      "articles"
    );
    console.log(
      "Sample article from Supabase:",
      data?.[0]
        ? {
            id: data[0].id,
            title: data[0].title,
    ...
    const mappedArticles = (data || []).map((article: any) => {
      ...
      return {
        ...
        imageUrl: validateImageUrl(
          article.image_url || article.image,
          article.title
        ), // Map 'image_url' or 'image' column to 'imageUrl'
    ...
    console.log(
      "Mapped articles with featured flags:",
      mappedArticles.map((a: any) => ({
        id: a.id,
        title: a.title,
    ...
    console.log(
      "🚀 [PRODUCTION] Skipping cache update - always fetch fresh data"
    );
    // articlesCache = mappedArticles
    // cacheTimestamp = now
...
export function testImageFieldSafeguards() {
  ...
  console.log(
    "Test 4 - Valid image URL:",
    validateImageUrl("data:image/jpeg;base64,/9j/4AAQ...", "Test Article")
  );
  console.log(
    "Test 5 - Null image URL:",
    validateImageUrl(null, "Test Article")
  );
  console.log(
    "Test 6 - Empty image URL:",
    validateImageUrl("", "Test Article")
  );
...
export async function getArticleBySlug(slug: string): Promise<Article | null> {
  try {
    ...
    if (!supabase) {
      ...
      try {
        ...
        const exactMatch = data.find((article: any) => {
          ...
          if (isMatch) {
            console.log(
              `✅ EMERGENCY: Found exact match for slug "${slug}": "${article.title}" -> "${articleUrlTitle}"`
            );
          }
          return isMatch;
        });
        ...
        if (exactMatch) {
          console.log(
            "✅ EMERGENCY: Successfully fetched article using emergency client"
          );
          return {
            ...exactMatch,
        ...
        console.error(
          "Emergency Supabase client creation failed:",
          emergencyError
        );
        console.log("Falling back to file system");
        const fileArticles = (await fileArticlesModule)
    ...
    console.log(
      "🚀 SMART FALLBACK: Trying Supabase first, file system fallback if slow"
    );

    // Try Supabase with timeout
    try {
      const fields = ensureImageFields(
        "id, title, excerpt, content, category, categories, location, author, tags, type, status, created_at, updated_at, trending_home, trending_edmonton, trending_calgary, featured_home, featured_edmonton, featured_calgary"
      );
      ...
      const exactMatch = data.find((article: any) => {
        ...
        if (isMatch) {
          console.log(
            `✅ SUPABASE: Found exact match for slug "${slug}": "${article.title}"`
          );
        }
        return isMatch;
      });
      ...
      if (!exactMatch) {
        console.log(
          `⚠️ SUPABASE: No exact match found, trying partial matching for slug: "${slug}"`
        );
        const partialMatch = data.find((article: any) => {
          ...
          const matchingWords = slugWords.filter(
            (word) =>
          ...
          if (isPartialMatch) {
            console.log(
              `✅ SUPABASE: Found partial match (${Math.round(
                matchPercentage * 100
              )}%) for slug "${slug}": "${article.title}"`
            );
          }
        ...
        if (partialMatch) {
          ...
          return {
            ...partialMatch,
            imageUrl: validateImageUrl(
              partialMatch.image_url || partialMatch.image,
              partialMatch.title
            ),
      ...
      if (exactMatch) {
        ...
        return {
          ...exactMatch,
          imageUrl: validateImageUrl(
            exactMatch.image_url || exactMatch.image,
            exactMatch.title
          ),
          date: exactMatch.created_at,
          trendingHome: exactMatch.trending_home || false,
      ...
      console.log(
        "⚠️ SUPABASE: Failed or slow, falling back to file system:",
      ...
      try {
        ...
        if (!fileArticle) {
          console.log(
            `⚠️ FILE SYSTEM: No exact match found, trying partial matching for slug: "${slug}"`
          );
          fileArticle = fileArticles.find((article: any) => {
            ...
            if (isPartialMatch) {
              console.log(
                `✅ FILE SYSTEM: Found partial match (${Math.round(
                  matchPercentage * 100
                )}%) for slug "${slug}": "${article.title}"`
              );
            }
        ...
        if (fileArticle) {
          console.log(
            `✅ FILE SYSTEM: Found article in file system for slug "${slug}": "${fileArticle.title}"`
          );
          return {
            ...fileArticle,
...
export async function getArticleById(id: string): Promise<Article | null> {
  try {
    ...
    if (shouldUseFileSystemFirst() && fileArticlesModule) {
      console.log(
        "🚀 SPEED: Using file system as primary source for article by ID"
      );
      try {
        ...
        console.warn(
          "⚠️ File system failed for article by ID, falling back to Supabase:",
          fileError
        );
      }
    } else {
...
export async function createArticle(
  ...
  try {
    ...
    try {
      ...
      console.warn(
        "⚠️ Auto-sync failed, but Supabase creation was successful:",
        syncError
      );
...
export async function updateArticle(
  ...
  try {
    ...
    try {
      ...
      console.warn(
        "⚠️ Auto-sync failed, but Supabase update was successful:",
        syncError
      );
...
export async function deleteArticle(id: string): Promise<void> {
    ...
    try {
      ...
      if (isProduction) {
        ...
        try {
          await fetch(
            `${
              process.env.VERCEL_URL || "https://culturealberta.com"
            }/api/revalidate`,
            {
              method: "POST",
              headers: {
          ...
          console.warn(
            "⚠️ Revalidation failed, but Supabase deletion was successful:",
            revalidateError
          );
        }
      } else {
        ...
        try {
          ...
          console.warn(
            "⚠️ Auto-sync failed, but Supabase deletion was successful:",
            syncError
          );
        }
      }
  ...
  if (!isProduction) {
    ...
    console.error(
      "❌ Cannot delete article: Supabase not available and file system is read-only in production"
    );
    throw new Error(
      "Cannot delete article: Supabase not available and file system is read-only in production"
    );
